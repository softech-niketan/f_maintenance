...
class AdminController extends CI_Controller
{
	...
	public function index()
	{
				...
				if ($assign_pm_group_data) {
					...
					foreach ($assign_pm_group_data as $a) {
						if ($a->planeed_pm_date) {
							...
							if ($month_id == substr($a->planeed_pm_date, 5, 2)) {
								$jj = 1;
		...
		if ($i != 1) {
			...
			$data['mttr'] = round($total_bd_hours / ($total_bd_failure));
		}
		...
		$sum_main =  $this->Crud_model->query_sinegle("SELECT COUNT(id) AS SUM  FROM pm_request WHERE status != 'checksheet_completed' ");
		$data['PM_pending'] = $sum_main[0]->SUM;
		$sum_main2 =  $this->Crud_model->query_sinegle("SELECT COUNT(id) AS SUM  FROM breakdown_request WHERE status != 'request_closed' ");
	...
	public function pending()
	{
		...
		$role_management_data = $this->db->query('SELECT *   FROM ' . $type . ' WHERE status !="FeedBack"');
	...
	public function i_work_request_pending()
	{
		...
		$role_management_data = $this->db->query('SELECT * FROM improvement_request WHERE status !="completed"');
		$data['breakdown_pending'] = $role_management_data->result();
	...
	public function change_status_pm_checksheet()
	{
		...
		if ($result) {
			...
			$data = array(
				...
				"status" => "Maintance Feedback Added",
				"new_pm_date" => "",
				"new_pm_time" => "",
				"feedback" => "",
				"postpone_feedback" => "",
	...
	public function login_user()
	{
		...
		if ($this->form_validation->run() == False) {
			...
			$result = $this->Crud_model->get_data_by_id_multile("user", $data);
			if ($result == 0) {
				$data3 = array(
